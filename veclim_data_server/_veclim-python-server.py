...
import veclim_data_server._fun_server as _fun_server

fcast_time = None
def set_fcast_time():
    global fcast_time
    #
    now = _fun_server.pandas.Timestamp.today()
    if (fcast_time == None) or ((now - fcast_time).days > 0):
        _fun_server.load_forecast_var(reload=True)
        fcast_time = now
...
def respondTiles(date0,date1,pr_x,pr_y,pr_z,pr_v,start_response):
    _fun_server.load_tiles()
    v_label = pr_v
    if ((date0 != None) and (date1 != None)):
        v_label += '_dates'
        ret = _fun_server.load_tiles_dates(v_label,date0,date1)
        if ret:
    ...
    if ((pr_z == None) or 
        (pr_x == None) or 
        (pr_y == None) or
        (v_label not in _fun_server.tile_dat)):
            ret = {
                key: {
                    'colors': _fun_server.tile_dat[key]['clscl'],
                    'labels': _fun_server.tile_dat[key]['cllbl']
                }
                for key in _fun_server.tile_dat
            }
            response_body = json.dumps(ret)
            return returnResponse(start_response, response_body)                        
    #
    buff = _fun_server.tile_dat[v_label]['fun'](_fun_server.tile_dat[v_label]['dat'], 
                                            pr_z, pr_x, pr_y, 
                                            cmap=_fun_server.tile_dat[v_label]['cmap'], 
                                            norm=_fun_server.tile_dat[v_label]['norm'],
                                            label=_fun_server.tile_dat[v_label]['label'])
    response_body = buff
...
def respondAlbopictus(date0,date1,lon,lat,timeseries,meteo_key,sim_key,fcast_key,risk_key,start_response):
    simclm = _fun_server.get_decadal(lon,lat,date0,date1,ts=timeseries)
    if not simclm:
        return returnResponse(start_response, empty_response)
    #
    vec = _fun_server.presence.search(lon,lat)
    #
    ret = {
...
def respondPapatasi(date0,date1,lon,lat,timeseries,sim_key,start_response):
    simclm = _fun_server.get_sandfly(lon,lat,date0,date1,ts=timeseries)
    if not simclm:
        return returnResponse(start_response, empty_response)
    #
    ret = {
...
def application(environ, start_response):
    ...
    if 'date' in parameters:
        ...
        date0 = _fun_server._fun_clim.datetime.strptime(date0, "%Y-%m-%d")
        date1 = date0
    elif 'dates' in parameters:
        dates = escape(parameters.get('dates', [''])[0]).split(':')
        date0 = _fun_server._fun_clim.datetime.strptime(dates[0], "%Y-%m-%d")
        date1 = _fun_server._fun_clim.datetime.strptime(dates[1], "%Y-%m-%d")
    else:
        date0 = None
        date1 = None
        dates = None
    ...
    if 'vec' in parameters:
        ...
        vector = 'albopictus'
    if not (vector in _fun_server.veclist):
        return returnResponse(start_response, empty_response)
    #
