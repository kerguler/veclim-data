...
filename = "%s/surveillance/presence/albosurv_matrix" %(DIR_DATA)
presence = presenceAlbopictus()
vabun = VectAbundance()
aimsurv = AIMsurv()
vbase = VectorBase()
...
def getSurv(lon, lat):
    ...
    #
    return {
        'vabun': _getSurv(vabun.vb,lon,lat),
        'aimsurv': _getSurv(aimsurv.vb,lon,lat),
        'vbase': _getSurv(vbase.vb,lon,lat)
    }
...
def getMatrix(res=[0.125,0.125]):
    try:
        mat = numpy.load(filename + ".npy")
    except:
        obj = _getShpD()
        #
        tran = [
